...
class gammaFilter(object):
    ...
    def __call__(self, data1):
        ...
        for channel in range(data.shape[0]):
            ...
            out[channel, :] = self.butter_highpass_filter(out[channel, :], self.highcut, self.fs)
            out[channel, :] = self.firwin_bandpass_filter(out[channel, :], self.ntaps, self.band[0], self.band[1],
                                                          self.fs)
...
class Energy_Wavelet(object):
    def __init__(self, lowcut=0.16, highcut=78, bands=[[0.17, 4], [4, 8], [8, 14], [14, 32], [32, 64], [64, 78]],
                 fs=160, ntaps=128):
        self.lowcut = lowcut
        self.highcut = highcut
        self.bands = bands
    ...
    def __call__(self, data1):
        data = copy.deepcopy(data1)
        out = np.zeros([data.shape[0], len(self.bands) * 4])

        for channel in range(data.shape[0]):
            energys = []
            total_energy = 0
            for i in range(len(self.bands)):
                temp = self.butter_lowpass_filter(data[channel, :], self.lowcut, self.fs)
                temp = self.butter_highpass_filter(temp, self.highcut, self.fs)
                energys.append(sum(x ** 2 for x in
                                   self.firwin_bandpass_filter(temp, self.ntaps, self.bands[i][0], self.bands[i][1],
                                                               self.fs)))
                total_energy = total_energy + energys[-1]
            for i in range(len(self.bands)):
                out[channel, i * 4] = energys[i]
                out[channel, i * 4 + 1] = (energys[i] / total_energy) * 100
                out[channel, i * 4 + 2] = math.log((energys[i] / total_energy) * 100)
                out[channel, i * 4 + 3] = abs(math.log((energys[i] / total_energy) * 100))
