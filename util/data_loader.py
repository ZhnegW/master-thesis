...
from util.transforms import filterBank, gammaFilter, MSD, Energy_Wavelet
...
class BCI2aDataset(Dataset):
    def get_subject_data(self, subject, path, No_channels, No_trials, Window_Length, sample_ratio=1, label=1,
                         train='train'):
        No_valid_trial = 0
            ...
            for trial in range(0, run_trial.size // sample_ratio):
                if run_artif[trial] == 0:
                    data_return[No_valid_trial, :, :] = np.transpose(
                        run_X[(int(run_trial[trial] + 1.5 * 250)):(int(run_trial[trial] + 6 * 250)), :No_channels])
    ...
    def __init__(self, subject, path, train='train', transform=None, channels=None, select_channel=False,
                 preprocess=False):
        if channels is None:
        ...
        if train == 'train':
            self.data_return, self.class_return = self.get_subject_data(subject, path, No_channels, No_trials,
                                                                        Window_Length, sample_ratio=1, label=1,
                                                                        train=train)
            ...
            for i in [x for x in range(1, 5) if x != subject]:
                negative_data, negative_class = self.get_subject_data(i, path, No_channels, No_trials, Window_Length,
                                                                      sample_ratio=2, label=0, train=train)
                if preprocess:
    ...
    def KMAR_PIC(self, data):
        ...
        # local var
        local_var = np.zeros((Y.shape[0], Y.shape[1] // 250))
        for i in range(0, Y.shape[1] // 250 - 1):
            local_var[:, i] = np.var(Y[:, i * 250:(i + 1) * 250], axis=1)
        Y_var = np.mean(local_var, axis=1, keepdims=True)
        local_var15 = np.zeros((Y.shape[0], Y.shape[1] // 40))
        for i in range(0, Y.shape[1] // 62 - 1):
            local_var15[:, i] = np.var(Y[:, i * 62:(i + 1) * 62], axis=1)
        Y_var15 = np.mean(local_var15, axis=1, keepdims=True)
        # local_skew
        local_skew = np.zeros((Y.shape[0], Y.shape[1] // 250))
        for i in range(0, Y.shape[1] // 250 - 1):
            local_skew[:, i] = skew(Y[:, i * 250:(i + 1) * 250], axis=1)
        Y_skew = np.mean(local_skew, axis=1, keepdims=True)
        local_skew15 = np.zeros((Y.shape[0], Y.shape[1] // 62))
        for i in range(0, Y.shape[1] // 62 - 1):
            local_skew15[:, i] = skew(Y[:, i * 62:(i + 1) * 62], axis=1)
    ...
    #     for i in range(data.shape[0]):
    #         trial_channel = channelSelection.select(data[i, :, :], 12)
    #         for chan in trial_channel:
    #             channelCount[chan] = channelCount[chan] + 1
    #     self.channel_selected = np.argpartition(channelCount, -12)[-12:]
    #
    # def channels(self):
...
class PhysioDataset(Dataset):
    ...
    def __init__(self, subject, path, train='train', transform=None, select_channel=False, preprocess=False,
                 channels=None, use_channel_no=0):
        if channels is None:
...
if __name__ == '__main__':
    ...
    filterTransform = gammaFilter(band=[8, 45])

    train_data = PhysioDataset(1, "../data/physionet/eegmmidb/files", train='train',
                               transform=filterTransform, use_channel_no=64, preprocess=False, select_channel=False)
