...
from model.vsmscnn3 import VSMSCNN
...
if __name__ == '__main__':
    ...
    if config["transform"]["name"] == "nBand":
        filterTransform = filterBank([[4,8],[8,13],[13,30],[30, 40]], 160)
    ...
    if config["dataset"]["name"] == "PhysioDataset":
        train_data = PhysioDataset(subject=config["train"]["subject"], path=config["dataset"]["location"],
                                   train="train", transform=filterTransform, channels=config["channel"]["select"],
                                   use_channel_no=config["channel"]["number"], preprocess=config["dataset"]["preprocess"])
        ...
        channels = train_data.channels()  # get the channel number

        valid_data = PhysioDataset(subject=config["train"]["subject"], path=config["dataset"]["location"],
        ...
        size_valid = len(valid_dataloader.dataset)
    ...
    for t in range(epochs):
        print(f"Epoch {t + 1}\n-------------------------------")
        train_losses, accuracy = train(train_dataloader, model, loss_fn, optimizer, size=size_train)
        train_loss = np.average(train_losses)
        exp_lr_scheduler.step()
        epoch_acc, valid_losses = valid(valid_dataloader, model, loss_fn, size=size_valid)
