...
if __name__ == '__main__':
    ...
    if config["dataset"]["name"] == "PhysioDataset":
        train_data = PhysioDataset(subject=config["train"]["subject"], path=config["dataset"]["location"],
                                   train="train", transform=filterTransform, select_channel=config["channel"]["select"],
        ...
        inter_test_data = PhysioDataset(subject=config["train"]["subject"], path=config["dataset"]["location"],
                                        train="inter_test", transform=filterTransform, channels=channels,
                                        preprocess=config["dataset"]["preprocess"])
        if config["evaluate"]["batch_size"] == "all":
        ...
        train_data = BCI2aDataset(subject=config["train"]["subject"], path=config["dataset"]["location"], train="train",
        ...
        intra_test_data = BCI2aDataset(subject=config["train"]["subject"], path=config["dataset"]["location"],
                                       train="intra_test", transform=filterTransform, channels=channels)
        if config["evaluate"]["batch_size"] == "all":
            batch_size = len(intra_test_data)
        else:
        ...
        inter_test_data = BCI2aDataset(subject=config["train"]["subject"], path=config["dataset"]["location"],
                                       train="inter_test", transform=filterTransform, channels=channels)
        if config["train"]["batch_size"] == "all":
            batch_size = len(train_data)
        else:
